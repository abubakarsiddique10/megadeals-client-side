...
const VerticleCard = ({ product }) => {
    ...
    const { image, productName, stock, price, ratings, percantage, discountPrice, _id } = product;

    const handleCard = (id) => {
        navigate(`/productDetails/${id}`)
    }
    return (
        <div onClick={() => handleCard(_id)} className="product-card border relative hover:border-[#F94073] transition-all duration-400 max-w-[246px] cursor-pointer">
            ...
            <button className="like-btn opacity-0 absolute flex justify-center items-center rounded-full h-9 w-9 bg-[#2D3054] top-5 right-3 flex justify-center items-center text-[14px]"><img src="https://i.ibb.co/Q9HK0qp/heart-4.png" className="w-4 mt-1" /></button>
        </div>
    )
}
